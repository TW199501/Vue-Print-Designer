  ...
  <div class="relative">
    ...
    <div v-if="showPaperSettings" class="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-xl rounded-lg p-4 z-[1000]">
      ...
      <div class="mt-3 flex items-center justify-between">
        <span class="text-sm text-gray-700 dark:text-gray-200 font-medium">{{ t('editor.allowDragOutsideCanvas') }}</span>
        <button
          @click="store.setAllowDragOutsideCanvas(!store.allowDragOutsideCanvas)"
          class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2"
          :class="store.allowDragOutsideCanvas ? 'bg-blue-600 dark:bg-blue-500' : 'bg-gray-200 dark:bg-gray-700'"
        >
          <span class="sr-only">Toggle drag outside canvas</span>
          <span
            aria-hidden="true"
            class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white dark:bg-gray-100 shadow ring-0 transition duration-200 ease-in-out"
            :class="store.allowDragOutsideCanvas ? 'translate-x-5' : 'translate-x-0'"
          />
        </button>
      </div>
